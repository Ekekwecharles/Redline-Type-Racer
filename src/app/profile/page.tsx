...
export default function ProfilePage() {
  ...
  const avgAcc = profile.racesPlayed ? Math.round(profile.accSum / profile.racesPlayed) : 100;

  return (
    <div className="flex flex-col gap-3.5">
      {isGuest && (
        <div className="text-[11px] text-dim bg-asphalt-3 border border-dashed border-line rounded-lg px-3 py-2">
          You&apos;re racing as a guest — progress is saved to this browser only.{" "}
      ...
      <div className="bg-asphalt-2 border border-line rounded-2xl p-5">
        <h2 className="font-display text-sm tracking-widest uppercase text-fog mb-3.5">Driver Profile</h2>
        <div className="flex items-center gap-3.5 flex-wrap">
          ...
          <span className="text-xs text-dim">
            Level <b className="text-white">{profile.level}</b> · <b className="text-white">{xpIntoLevel}</b> / {xpNeeded} XP
          </span>
        </div>
        <div className="h-2.5 bg-asphalt-3 rounded-full overflow-hidden mt-2">
          <div className="h-full bg-gradient-to-r from-violet to-cyan" style={{ width: `${Math.min(100, (xpIntoLevel / xpNeeded) * 100)}%` }} />
        </div>
        ...
        <div className="grid gap-2.5 mt-3.5" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))" }}>
          {[
            ...
            <div key={s.label} className="bg-asphalt-3 border border-line rounded-xl p-3">
              <div className="font-display text-xl">{s.value}</div>
              <div className="text-[10px] text-dim uppercase tracking-wider">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-asphalt-2 border border-line rounded-2xl p-5">
        <h2 className="font-display text-sm tracking-widest uppercase text-fog mb-3.5">Achievements</h2>
      ...
      <div className="bg-asphalt-2 border border-line rounded-2xl p-5">
        <h2 className="font-display text-sm tracking-widest uppercase text-fog mb-3.5">Recent Races</h2>
        {profile.history.length === 0 ? (
          <p className="text-[11px] text-dim">No races yet. Head to the Race tab.</p>
        ) : (
          <div>
            {profile.history.map((h, i) => (
              <div key={i} className="flex justify-between text-xs text-fog py-1.5 border-b border-line last:border-0">
                <span>
                  {h.date} · {h.mode}
                </span>
                <span>
