...
export default function HUD({ wpm, acc, combo, penalty }: HUDProps) {
  ...
  return (
    <div className="flex gap-2.5 flex-wrap mb-3.5">
      {items.map((item) => (
        <div key={item.label} className="flex-1 min-w-[100px] bg-asphalt-2 border border-line rounded-xl px-3.5 py-2.5">
          <div className="font-display text-[10px] tracking-widest text-dim uppercase">{item.label}</div>
